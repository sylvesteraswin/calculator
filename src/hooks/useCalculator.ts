    ...
    e => {
      ...
      switch (true) {
        ...
        case equals === clickedValue: {
          ...
          // Extract computation logic to avoid nested setState
          const processCalculation = (currentValue: string[]) => {
          ...
          // Use useRef to store calculation result and avoid direct state access
          setValue(prev => {
            const result = processCalculation(prev);
            calculationResultRef.current = result;
            return result.newValue;
          });

          // Update last operation separately using the stored result
